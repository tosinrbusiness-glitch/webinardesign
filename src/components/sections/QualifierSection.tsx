...
const forYou = [
  "You're renting in Philadelphia right now",
  "You're tired of paying someone else's mortgage",
  "You want to build generational wealth for your family",
  "You're ready to take action — not just talk about it",
  "You believe homeownership is your RIGHT",
];

const notForYou = [
  "You're looking for a 'get rich quick' scheme",
  "You're not willing to attend a free webinar",
  "You have no interest in building real wealth",
  "You think the system can't work for you",
  "You're comfortable funding your landlord's retirement",
];
    ...
    >
      <div className="relative z-10 container mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <p className={`text-[#b87aff] font-heading text-xs tracking-[0.35em] uppercase mb-4 transition-all duration-500 ${isVisible ? "opacity-100" : "opacity-0"}`}>Are You Serious?</p>
          <h2 className={`font-heading text-4xl sm:text-5xl font-bold uppercase leading-tight transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <span className="text-foreground">IF YOU'RE SERIOUS —</span><br />
            <span className="bg-gradient-to-r from-[#f5c518] to-[#ffdf6b] bg-clip-text text-transparent" style={{ filter: 'drop-shadow(0 0 15px rgba(245, 197, 24, 0.2))' }}>THIS IS FOR YOU.</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          ...
          <div className={`rounded-2xl p-[1px] transition-all duration-700 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`} style={{ background: 'linear-gradient(145deg, rgba(245, 197, 24, 0.5), rgba(245, 197, 24, 0.1))' }}>
            <div className="rounded-2xl p-8 h-full" style={{ background: 'linear-gradient(145deg, rgba(30, 16, 64, 0.7), rgba(15, 8, 35, 0.9))' }}>
              <h3 className="font-heading text-xl font-bold uppercase mb-6 bg-gradient-to-r from-[#f5c518] to-[#ffdf6b] bg-clip-text text-transparent">✅ This Webinar Is For You If...</h3>
